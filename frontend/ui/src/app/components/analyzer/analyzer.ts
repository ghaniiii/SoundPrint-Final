...
export class AnalyzerComponent implements OnDestroy {
  ...
  toggleUpload(): void {
    ...
    const url = this.analysisResult?.track?.previewUrl;
    if (!url) return;
  ...
  confirmAndAnalyzeSelection(): void {
    if (!this.audioBuffer || !this.uploadedFile) return;
    ...
    // send this file to the backend for analysis
    ...
    this.api.analyzeTrack(file, this.uploadedFile).subscribe({
      ...
      error: (err) => {
        console.error(err);
        this.errorMessage = err?.error?.message || 'Analysis failed';
        this.isAnalyzing = false;
      }
    });
  }
