...
# Windows consoles default to cp1252 and crash on emoji log lines.
    ...
    try:
        ...
        def to_preview(url):
            ...
            if str(url).startswith("http"):
                return url
            return f"{base_url}{str(url).replace(chr(92), '/')}"
        ...
        response = {
            ...
            "similarTracks": [
                {
                    **t,
                    "previewUrl": to_preview(t.get("previewUrl"))
                }
        ...
        print(tb, flush=True)
        try:
            (BASE_DIR / "recommend_error.log").write_text(tb, encoding="utf-8")
        except Exception:
            pass
